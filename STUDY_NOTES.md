# 📚 SOC Level 1 Analyst — Study Notes Index

> A complete index of all study notes for the SOC L1 Analyst Course.
> Follow the modules in order for the best learning experience.

---

## 🗺️ Quick Navigation

| Phase | Modules | Topic |
|-------|---------|-------|
| **Foundation** | 01 – 06 | SOC basics, team, tools, alerts |
| **Core Skills** | 07 – 09 | Triage, verdicts, investigation |
| **Context Building** | 10 – 14 | Inventory, threat intel, playbooks |
| **Investigation** | 15 – 17 | Enrichment, SIEM, alert scenarios |
| **Communication** | 18 – 21 | Reporting, escalation, metrics |
| **Professional** | 22 – 23 | Improvement, resilience |
| **Practicals** | 24 – 28 | Labs, capstone, final assessment |

---

## 📖 Module Study Notes

### 🔵 Phase 1 — Foundation

| # | Module | Study Notes |
|---|--------|-------------|
| 01 | Introduction to SOC | [📄 Read Notes](modules/module-01-introduction-to-soc/index.md) |
| 02 | SOC Team Structure | [📄 Read Notes](modules/module-02-soc-team-structure/index.md) |
| 03 | SOC Tools & Environment | [📄 Read Notes](modules/module-03-soc-tools-and-environment/index.md) |
| 04 | Events, Logs & Alerts | [📄 Read Notes](modules/module-04-events-logs-and-alerts/index.md) |
| 05 | Alert Properties | [📄 Read Notes](modules/module-05-alert-properties/index.md) |
| 06 | Alert Prioritisation | [📄 Read Notes](modules/module-06-alert-prioritisation/index.md) |

---

### 🟢 Phase 2 — Core Skills

| # | Module | Study Notes |
|---|--------|-------------|
| 07 | Alert Triage Fundamentals | [📄 Read Notes](modules/module-07-alert-triage-fundamentals/index.md) |
| 08 | Alert Verdicts | [📄 Read Notes](modules/module-08-alert-verdicts/index.md) |
| 09 | Investigation Methodology | [📄 Read Notes](modules/module-09-investigation-methodology/index.md) |

---

### 🟡 Phase 3 — Context Building

| # | Module | Study Notes |
|---|--------|-------------|
| 10 | Identity Inventory | [📄 Read Notes](modules/module-10-identity-inventory/index.md) |
| 11 | Asset Inventory | [📄 Read Notes](modules/module-11-asset-inventory/index.md) |
| 12 | Threat Intelligence Lookups | [📄 Read Notes](modules/module-12-threat-intelligence-lookups/index.md) |
| 13 | Network Diagrams | [📄 Read Notes](modules/module-13-network-diagrams/index.md) |
| 14 | Workbooks, Playbooks & Runbooks | [📄 Read Notes](modules/module-14-workbooks-playbooks-runbooks/index.md) |

---

### 🟠 Phase 4 — Deep Investigation

| # | Module | Study Notes |
|---|--------|-------------|
| 15 | Enrichment Process | [📄 Read Notes](modules/module-15-enrichment-process/index.md) |
| 16 | SIEM Investigation | [📄 Read Notes](modules/module-16-siem-investigation/index.md) |
| 17 | Common Alert Scenarios | [📄 Read Notes](modules/module-17-common-alert-scenarios/index.md) |

---

### 🔴 Phase 5 — Communication & Reporting

| # | Module | Study Notes |
|---|--------|-------------|
| 18 | Alert Reporting | [📄 Read Notes](modules/module-18-alert-reporting/index.md) |
| 19 | Alert Escalation | [📄 Read Notes](modules/module-19-alert-escalation/index.md) |
| 20 | SOC Communication | [📄 Read Notes](modules/module-20-soc-communication/index.md) |
| 21 | SOC Metrics & Objectives | [📄 Read Notes](modules/module-21-soc-metrics-objectives/index.md) |

---

### 🟣 Phase 6 — Professional Growth

| # | Module | Study Notes |
|---|--------|-------------|
| 22 | SOC Improvement & Learning | [📄 Read Notes](modules/module-22-soc-improvement-learning/index.md) |
| 23 | Professional Skills & Resilience | [📄 Read Notes](modules/module-23-professional-skills-resilience/index.md) |

---

### ⚫ Phase 7 — Practical Labs & Assessment

| # | Module | Study Notes |
|---|--------|-------------|
| 24 | Beginner Practical Lab | [📄 Read Notes](modules/module-24-beginner-practical-lab/index.md) |
| 25 | Intermediate Practical Lab | [📄 Read Notes](modules/module-25-intermediate-practical-lab/index.md) |
| 26 | Advanced Practical Lab | [📄 Read Notes](modules/module-26-advanced-practical-lab/index.md) |
| 27 | Capstone Project | [📄 Read Notes](modules/module-27-capstone-project/index.md) |
| 28 | Final Assessment | [📄 Read Notes](modules/module-28-final-assessment/index.md) |

---

## 📂 Additional Resources

| Resource | Link |
|----------|------|
| 📘 Course Overview | [docs/course-overview.md](docs/course-overview.md) |
| 🗺️ Course Roadmap | [docs/course-roadmap.md](docs/course-roadmap.md) |
| 🎯 Learning Objectives | [docs/learning-objectives.md](docs/learning-objectives.md) |
| 📋 Prerequisites | [docs/prerequisites.md](docs/prerequisites.md) |
| 📖 Glossary | [docs/glossary.md](docs/glossary.md) |
| 🏆 Career Path | [docs/soc-l1-career-path.md](docs/soc-l1-career-path.md) |
| 🛡️ Playbooks | [playbooks/](playbooks/) |
| 📊 Diagrams | [diagrams/](diagrams/) |

---

## ✅ Study Progress Tracker

Use this checklist to track your progress:

- [ ] Module 01 — Introduction to SOC
- [ ] Module 02 — SOC Team Structure
- [ ] Module 03 — SOC Tools & Environment
- [ ] Module 04 — Events, Logs & Alerts
- [ ] Module 05 — Alert Properties
- [ ] Module 06 — Alert Prioritisation
- [ ] Module 07 — Alert Triage Fundamentals
- [ ] Module 08 — Alert Verdicts
- [ ] Module 09 — Investigation Methodology
- [ ] Module 10 — Identity Inventory
- [ ] Module 11 — Asset Inventory
- [ ] Module 12 — Threat Intelligence Lookups
- [ ] Module 13 — Network Diagrams
- [ ] Module 14 — Workbooks, Playbooks & Runbooks
- [ ] Module 15 — Enrichment Process
- [ ] Module 16 — SIEM Investigation
- [ ] Module 17 — Common Alert Scenarios
- [ ] Module 18 — Alert Reporting
- [ ] Module 19 — Alert Escalation
- [ ] Module 20 — SOC Communication
- [ ] Module 21 — SOC Metrics & Objectives
- [ ] Module 22 — SOC Improvement & Learning
- [ ] Module 23 — Professional Skills & Resilience
- [ ] Module 24 — Beginner Practical Lab
- [ ] Module 25 — Intermediate Practical Lab
- [ ] Module 26 — Advanced Practical Lab
- [ ] Module 27 — Capstone Project
- [ ] Module 28 — Final Assessment 🎓

---

**Good luck on your SOC L1 journey! 🛡️**
